...
class Output():
    @staticmethod
    def I():
        print("(i)   A x (B ∪ C) = (A x B) ∪ (A x C)\n            B ∪ C = {{{}}} ∪ {{{}}} = {{{}}}".format(str(B)[1:-1],str(C)[1:-1],str(sorted(list(set().union(B,C))))[1:-1]))
        print("      A x (B ∪ C) = {{{}}} x {{{}}} = {{{}}}   ...(1)".format(str(A)[1:-1],str(sorted(list(set().union(B,C))))[1:-1],",".join(repr(_) for _ in list(itertools.product(A,sorted(list(set().union(B,C))))))))
...
def assign():
    ...
    print("(i) A x (B ∪ C) = (A x C) (ii) A x (B ∩ C) = (A x B) ∩ (A x C)")
assign()
